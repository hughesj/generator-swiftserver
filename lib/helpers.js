...
exports.codableHandlerNameInsert = function (detail, count) {
  // Return a name insert comprised from the input and output types and
  // a number to ensure uniqueness.
  //
  let paramType = detail.params[0] || undefined
  let responseType = detail.responses[0] || undefined
  let pname
  let rname
  var name

  if (paramType) {
    if (paramType['model'] && paramType['array']) {
      pname = 'ArrayOf' + paramType['model']
    } else if (paramType['model']) {
      pname = paramType['model']
    }
  }

  if (responseType) {
    if (responseType['model'] && responseType['array']) {
      rname = 'ArrayOf' + responseType['model']
    } else if (responseType['model']) {
      rname = responseType['model']
    }
  }
  ...
  if (!count[detail.method]) {
    count[detail.method] = {}
    count[detail.method][detail.variant] = 0
  }

  if (detail.method === 'delete') {
    name = 'Handler_' + count[detail.method][detail.variant].toString()
  } else {
    name = name + '_Handler_' + count[detail.method][detail.variant].toString()
  }
  // increment the count
  count[detail.method][detail.variant] += 1
